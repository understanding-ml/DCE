...
class DCESolver(BaseExplainer):
    ...
    def explain(
        ...
        if use_global_ranges and hasattr(self.data, 'continuous_ranges') and hasattr(self.data, 'categorical_ranges'):
            ...
            # Convert to index mapping format
            ...
            # Not using global ranges, clear global range info in strategy and fall back to X_prime ranges
            if hasattr(strategy, 'set_global_ranges'):
            ...
            if not use_global_ranges:
                ...
                print("⚠️ No global ranges found in data loader, using X_prime ranges")
        
        # Initialize logger data
        self.logger_data = []

        # Initialize containers to store X and y for each iteration
        self.iteration_X_list = []  # Store X for each iteration (denormalized)
        ...
        for i in range(max_iter):
            ...
            if save_results:
                ...
                # Collect iteration data based on switch
                if save_iterations:
                    # Save current iteration's X (denormalized)
                    X_current_denorm = self._denormalize_X(self.X, df_factual.columns)
                    self.iteration_X_list.append(X_current_denorm)

                    # Save current iteration's y values (flatten to 1D array)
            ...
            # Discard image generation callback
            # if callback:
            #     callback(self, i)

        print("Optimization End")

        # Record optimization end time and calculate duration
    ...
    def _denormalize_X(self, X_tensor, columns):
        ...
        # Use mean and std from data for denormalization
        ...
        # Recover data types (integers, etc.)
