...
class ResultSaver:
    ...
    def save_results(self, explainer, columns):
        ...
        try:
            ...
            # Save best Q and corresponding x,y as CSV (now with recovered/denormalized data)
            if hasattr(explainer, 'best_X') and hasattr(explainer, 'best_y'):
                # Save best_x with denormalized version
                ...
                # Save recovered data as best_x.csv
            ...
            # Save final round Q and corresponding x,y as CSV (now with recovered/denormalized data)
            if hasattr(explainer, 'final_X') and hasattr(explainer, 'final_y'):
                # Save final_x with denormalized version
                ...
                # Save recovered data as final_x.csv
            ...
            # Save X tables for each iteration (denormalized)
            ...
            # Save y matrix for all iterations
    ...
    def save_initial_data(self, explainer, df_factual, y_target):
        ...
        try:
            ...
            # ⚠️ CRITICAL: Convert x_true from standardized to original data before saving
            # df_factual is standardized data, need to inverse transform
            ...
            # Save x_true (factual data as original scale)
    ...
    def save_iteration_x_tables(self, iteration_X_list, save_dir):
        """
        Save X table for each iteration to separate CSV files
        Each file contains X for all samples in that iteration (denormalized)
        """
    ...
    def save_all_iterations_y(self, iteration_y_matrix, save_dir):
        ...
        Save y matrix for all iterations
        Format:
        - First column: sample_id (sample index 0, 1, 2, ...)
        - Following columns: iter_0, iter_1, iter_2, ... (y values for each iteration)
        - Each row represents one sample's y values across all iterations
        """
        # iteration_y_matrix: list of arrays, shape = [(n_samples,), (n_samples,), ...]
        # Convert to (n_samples, max_iter) matrix
        y_matrix = np.array(iteration_y_matrix).T  # Transpose: (max_iter, n_samples) -> (n_samples, max_iter)

        # Column names: iter_0, iter_1, ...
        columns = [f"iter_{i}" for i in range(y_matrix.shape[1])]

        # Create DataFrame with sample_id as first column
