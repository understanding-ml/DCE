    ...
    def generate_new_X(self, eta: float, num_trials: int, top_k: int = 1) -> torch.Tensor:
        ...
        for _ in range(num_trials):
            ...
            for idx in top_indices:
                ...
                # Categorical mutation with gradient guidance
                categorical_guidance_applied = False
                if self.use_gradient_guidance:
                    # Create temporary candidate for gradient computation
                    temp_cand = torch.zeros((1, explainer.X.shape[1]), device=explainer.device)
                    temp_cand[0] = parent1
                    categorical_guidance_applied = self._apply_gradient_guidance_to_categorical_features(temp_cand, eta, idx, 0)
                    if categorical_guidance_applied:
                        parent1 = temp_cand[0]
                
                if not categorical_guidance_applied:
                    # Original categorical mutation (fallback)
