...
class BayesianStrategy(OTGuidanceMixin):
            ...
            for idx in candidate_indices:
                ...
                for feat in explainer.explain_indices:
                    ...
                    if feat in self.continuous_global_ranges:
                        min_val = float(self.continuous_global_ranges[feat]['min'])
                        max_val = float(self.continuous_global_ranges[feat]['max'])
                    else:
                        min_val = float(explainer.X_prime[:, feat].min())
                        max_val = float(explainer.X_prime[:, feat].max())
                    
                    bounds.append(Real(
                        min_val - (1e-6 if min_val == max_val else 0),
                        max_val + (1e-6 if min_val == max_val else 0),
                        name=f"x_{feat}"
                    ))
