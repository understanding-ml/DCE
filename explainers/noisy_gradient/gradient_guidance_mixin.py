"""
Gradient Guidance Mixin for Strategy Classes - Noisy Gradient Method

This module provides noisy gradient guidance functionality
where final direction = guide_direction + β * noise
"""
...
class GradientGuidanceMixin:
    """
    Mixin class that provides noisy gradient guidance functionality for optimization strategies.
    
    This implementation uses: final_dir = guide_direction + β * noise
    where guide_direction is the negative gradient of term1 (SWD part) and noise
    is Gaussian random noise scaled by β (noise_beta parameter).
    """
    
    def __init__(self, explainer, use_gradient_guidance=False, noise_beta=0.1, random_state=None, **kwargs):
        """
        ...
        Args:
            ...
            noise_beta (float): Noise scaling factor for noisy gradient method (default: 0.1)
                               final_dir = guide_direction + β * noise
    ...
    def _sample_noisy_gradient(self, guide_direction, noise_beta, num_features):
        """
        Sample directions using noisy gradient method
        final_dir = guide_direction + β * noise
        """
        ...
        for i in range(batch_size):
            ...
            noise = self._rng.randn(num_features)
            
            # Noisy gradient: final_dir = guide_direction + β * noise
            noisy_direction = guide + noise_beta * noise
            
            # Normalize the final direction (optional, but helps with stability)
            if np.linalg.norm(noisy_direction) > 1e-8:
                noisy_direction = noisy_direction / np.linalg.norm(noisy_direction)
            else:
                # Fallback to pure gradient direction if noise cancels out gradient
                noisy_direction = guide
            
            directions.append(torch.from_numpy(noisy_direction).float().to(self.explainer.device))
    ...
    def _apply_gradient_guidance_to_continuous_features(self, cand, eta, ref_idx, idx):
        ...
        try:
            ...
            noisy_directions = self._sample_noisy_gradient(continuous_guide_direction, self.noise_beta, len(self.explainer.continuous_indices))
            
            # Apply guided sampling
            if noisy_directions.shape[0] > idx:
                for feat_idx, idx_feat in enumerate(self.explainer.continuous_indices):
                    if feat_idx < noisy_directions.shape[1]:
                        min_val = self.explainer.X_prime[:, idx_feat].min()
                        max_val = self.explainer.X_prime[:, idx_feat].max()
                        
                        # Use noisy gradient direction
                        direction = noisy_directions[idx, feat_idx]
                ...
                # Fallback to random sampling if noisy directions not available
                for idx_feat in self.explainer.continuous_indices:
                    self._apply_random_sampling_to_feature(cand, eta, ref_idx, idx, idx_feat)
            
            # Print success message when gradient guidance is used
            # print(f"[{self.__class__.__name__}] Successfully used noisy gradient guidance with β={self.noise_beta}")
            return True
