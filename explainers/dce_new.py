...
class DistributionalCounterfactualExplainer:
    def __init__(
        ...
        self.categorical_indices = [df_X.columns.get_loc(col) for col in categorical_columns]
        self.continuous_indices = [df_X.columns.get_loc(col) for col in continuous_columns]
        ...
        self.swd = SlicedWassersteinDivergence(
            self.X_prime[:, self.explain_indices].shape[1], n_proj=n_proj, random_state=random_state
        )
    ...
    def optimize(
        ...
        tol=1e-6,
        bootstrap=True,
        callback=None  # 新增一个 callback 参数
    ):
