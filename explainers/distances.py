...
class WassersteinDivergence:
    def __init__(self, reg=1, random_state=None):
        self.nu = None
        self.reg = reg
    ...
    def distance_interval(
        ...
        if bootstrap:
            return bootstrap_1d(
                y_s.detach().numpy(), y_t.detach().numpy(), delta=delta, alpha=alpha, random_state=self.random_state
            )
        else:
            return exact_1d(
...
class SlicedWassersteinDivergence:
    def __init__(self, dim: int, n_proj: int, reg=1, random_state=None):
        self.dim = dim
        self.n_proj = n_proj
        self.random_state = random_state
        
        # Set random seed for reproducible projection vectors
        if random_state is not None:
            np.random.seed(random_state)
        ...
        self.wd = WassersteinDivergence(random_state=random_state)
    ...
    def distance_interval(
        ...
        if bootstrap:
            return bootstrap_sw(X_s, X_t, delta=delta, alpha=alpha, swd=self, random_state=self.random_state)
        else:
            N = len(self.thetas)
            low = []
            up = []
...
def bootstrap_1d(x, y, delta, alpha, r=2, B=200, random_state=None):
    ...
    wd = WassersteinDivergence(random_state=random_state)
...
def bootstrap_sw(x, y, delta, alpha, swd, r=2, B=200, random_state=None):
    # Set random seed for reproducible bootstrap sampling
    if random_state is not None:
        np.random.seed(random_state)
