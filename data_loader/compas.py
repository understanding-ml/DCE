...
class CompasData:
    def __init__(self, test_size=0.2, seed=42, sample_num=50):
        ...
        # Use the same structure as baseline experiments
    ...
    def _preprocess(self):
        # Apply the same preprocessing as utils.datasets.py process_compas function
        
        # Drop missing values for screening arrest
        self.df = self.df.dropna(subset=['days_b_screening_arrest'])
        
        # Calculate time served (same logic as baseline)
        ...
        # Create the processed dataframe with the same structure as baseline
        processed_data = []
        ...
        # Convert ALL columns to float (as in baseline) to avoid numpy.object_ type
        for col in self.features:
            self.df[col] = pd.to_numeric(self.df[col], errors='coerce').astype(float)
        
        # Also convert target to float
        ...
        # Add compatibility attributes for old baseline code
        # self.data should be the raw, unstandardized data for AReS compatibility 
        # This will be updated in _standardize() to use data_raw
        
        # Create features_tree for AReS compatibility
        self.features_tree = {}
        
        # Group features by original categories for features_tree
        ...
        self.features_tree['Priors_Count'] = []  # Continuous feature
        self.features_tree['Time_Served'] = []   # Continuous feature
        
        # Add more compatibility attributes for AReS
        # Create categorical_features dictionary like in utils.datasets
        self.categorical_features = {
            self.name: ['Sex', 'Age_Cat', 'Race', 'C_Charge_Degree']
        }
        
        # Create continuous_features dictionary like in utils.datasets
        self.continuous_features = {
            self.name: ['Priors_Count', 'Time_Served']
        }
        
        # Update features to include target (as in utils.datasets)
        # This is needed for AReS compatibility
        self.features = self.features + [self.target_name]

    def _split(self):
        # Reset index before split to ensure proper alignment
        ...
        # Store the indices for mapping back to raw data
    ...
    def _standardize(self):
        ...
        # Keep raw unstandardized data for AReS (it needs original scale for binning)
        self.X_raw = self.X.reset_index(drop=True)  # Unstandardized feature data with reset index
        self.data_raw = self.df.reset_index(drop=True)  # Unstandardized full data including target with reset index
        
        # Update data to use raw unstandardized version for AReS compatibility
    ...
    def get_y_target(self):
        if TORCH_AVAILABLE:
            ...
            # Fallback using numpy for Beta distribution
