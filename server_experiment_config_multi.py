...
def calculate_metrics(results_directory: str, model, data):
    ...
    try:
        ...
        # Metric 3: OT Distance (using ORIGINAL scale as requested)
        try:
            ...
            # Use ORIGINAL scale for OT Distance calculation
            ot_distance = compute_distance(best_x_original, x_true_original)
            all_metrics['OT Distance'] = ot_distance
            print(f"   OT Distance (original scale): {ot_distance:.4f}")
        except Exception as e:
        ...
        try:
            ...
            print("   Percentile Differences (original scale):")
            for low, high, label in percentile_ranges:
                percentiles = np.arange(low, high, 0.1 if high - low <= 15 else 1)
                try:
                    # Use ORIGINAL scale for percentile comparison
                    diff_pct = compute_percentile_difference(best_x_original, x_true_original, percentiles)
        ...
        # 🆕 从 metadata.json 读取运行时间和 best_iter
        ...
        if os.path.exists(metadata_path):
            try:
                ...
                # 添加 best_iter（仅当 is_feasible 为 True 时）
                if is_feasible:
